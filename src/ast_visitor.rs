...
pub trait Visitable<T> {
    ...
    fn visit_if(&mut self, cond: &AstNode, then: &AstNode, else_: &AstNode)
        -> Result<T, String>;
    fn visit_let(&mut self, decl: &AstNode, body: &AstNode)
        -> Result<T, String>;
...
impl DebugVisitor {
    ...
    pub fn visit<T: Default>(&mut self, ast: &AstNode) -> Result<T, String> 
...
impl<T: Default> Visitable<T> for DebugVisitor {
    ...
    fn visit_fun_decl(&mut self, name: &str, clauses: &Vec<(AstPattern, Box<AstNode>)>, typ: &Option<Type>) -> Result<T, String> {
        let indent = " ".repeat(2 * self.debug_depth);
